...
* **Review rules explained in this document:** <p style="text-align: justify">As Fair Protocol is a novel implementation, we need to confirm the rules described above in a real scenario and review them if necessary for a future version of the application;</p>
...
* **Better unhappy flows on chat:** <p style="text-align: justify">When an Operator is offline during the last 7 blocks, a User will not get a response. In models that consider the context of previous messages for inference, it may make sense to have some kind of process that deals with this situation in case the person wants to change Operator immediately, without having to wait for the message from the Operator that is currently offline, for example cancelling the current sending and sending a new request to a new Operator. This way, it would be guaranteed that a certain exchange would not be ruined;</p>
