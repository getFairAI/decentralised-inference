...
import { createData, ArweaveSigner , DataItem, DataItemCreateOptions, JWKInterface, EthereumSigner } from '@dha-team/arbundles';
import {
  assert,
  isNotUndefined,
} from 'typed-assert';
 ...
 * with a stub
 */
const customDataSigner = async ({ data, tags, target, anchor}: Partial<signerParams>, createDataItem = (buf: Buffer) => new DataItem(buf) ) => {
  /**
