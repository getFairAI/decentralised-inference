...
const ModelCard = ({ modelTx, }: { modelTx: IEdge }) => {
  ...
  const tags = [
    ...
    { name: 'Model-Name', values: [ modelTx.node.tags.find(tag => tag.name === 'Model-Name')?.value]},
  ...
  const handleCardClick = () => {
    navigate(`/model/${encodeURIComponent(cardData?.txid || 'error')}/register`, { state: modelTx });
  };
