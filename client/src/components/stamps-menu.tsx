...
const StampsMenu: React.FC<StampsMenuProps> = (targetTx: StampsMenuProps) => {
    ...
    const tooltipText = 'When users "STAMP" the content, the STAMP Protocol gives creators and sponsors $STAMP Tokens every day.';
    ...
    const handleStampClick = useCallback(async () => {
      ...
      if(isStamped) {
        enqueueSnackbar(`You already stamped this ${targetTx.type.toLocaleLowerCase()}.`, {variant: 'info'});
        return;
      }
      if( currentAddress === '') {
      ...
      if(stampsRef.current && !isStamped) {
        try {
            ...
            setIsStamped(true);
            setIsSending(false);
            enqueueSnackbar(`The ${targetTx.type.toLocaleLowerCase()} has been stamped sucessffully!`, {variant: 'success'});
        } catch(error) {
          if (error instanceof Error) {
            enqueueSnackbar(error.message, { variant: 'error' });
          } else {
    ...
    return (
      <>
        ...
        >
          <span>
              <LoadingButton variant='contained' loading={isSending}  onClick={handleStampClick}>
