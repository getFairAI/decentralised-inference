...
import { Box, Checkbox, Divider, FormControlLabel, IconButton, TextField, Typography, useTheme } from '@mui/material';
...
const CustomTag = ({ name, value, handleRemove }: { name: string; value: string; handleRemove: (name: string, value: string) => void }) => {
...
const Configuration = ({
  ...
  const handleClose = useCallback(() => setConfigurationDraweOpen(false), [ setConfigurationDraweOpen ]);
  ...
  const handleRemove = useCallback((name: string, value: string) => {
  ...
  return <Box sx={{
    ...
    />
    <Box>
      <Divider textAlign="left"><Typography variant='h4'>Custom Tags</Typography></Divider>
    </Box>
    <Box display={'flex'} gap={'36px'} justifyContent={'space-between'}>
      <TextField
        inputRef={nameRef}
    ...
    {
      customTags.map(({ name, value }, index) => <CustomTag key={index} name={name} value={value} handleRemove={handleRemove}/>)
    }
    <FormControlLabel control={<Checkbox inputRef={keepConfigRef} />} label="Keep Configuration Between Prompts" />
  </Box>;
};
