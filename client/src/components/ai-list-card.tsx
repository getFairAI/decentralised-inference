...
import { Card, CardActionArea, CardHeader, CardMedia, CardContent, Typography, Box } from '@mui/material';
import { toSvg } from 'jdenticon';
import { MouseEvent, useMemo } from 'react';
import { useNavigate } from 'react-router-dom';


const AiListCard = ({ model, index, loading }: {model: IEdge, index: number, loading: boolean, error?: ApolloError}) => {
  ...
  return <Card sx={{
    background: 'linear-gradient(to bottom, rgba(118, 118, 118, 0.1) 2.17%, rgba(1, 1, 1, 0) 188.85%)',
    borderRadius: '10px',
  }}>
    <CardActionArea sx={{
      width: '100%',
    ...
    <CardMedia
      ...
      sx={{
        ...
        height: '100px',
        width: '100px',
        background: `linear-gradient(to top, #000000 10%, rgba(71, 71, 71, 0) 100%), url(${ loading ? '' : imgUrl})`,
