...
const ChatBubble = ({
  ...
  const showDayDivider = (el: IMessage, index: number) => {
    const daysDiffer = index < messages.length - 1 &&
      new Date(el.timestamp * secondInMS).getDay() !== new Date(messages[index + 1].timestamp * secondInMS).getDay();
    return <>
      {
        daysDiffer && <Divider textAlign='center'>
          <Typography
            sx={{
              fontStyle: 'normal',
              fontWeight: 300,
  ...
  const showResponseTimeoutFragment = () => {
    if (responseTimeout && !isWaitingResponse) {
      return <Container maxWidth={false} sx={{ paddingTop: '16px' }}>
        <Stack spacing={4} flexDirection='row'>
          <Box display={'flex'} flexDirection='column' margin='8px' width='100%'>
