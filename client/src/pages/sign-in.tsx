...
import { DEFAULT_TAGS, MODEL_CREATION_PAYMENT_TAGS, TAG_NAMES, U_CONTRACT_ID, U_LOGO_SRC } from '@/constants';
...
import { Box, Button, Container, Divider, IconButton, Slider, Step, StepContent, StepLabel, Stepper, TextField, Typography, useTheme } from '@mui/material';
  ...
  return <>
    ...
    <Box display={'flex'} gap={'8px'}>
      ...
      <Typography display={'flex'} gap={'8px'} alignItems={'center'}>
        <a href='https://docs.arweave.org/info/wallets/generating-cold-wallet' target='_blank' rel="noreferrer"><u>Learn how to create one.</u></a>
      </Typography>
    </Box>
...
const WalletNoFundsContent = () => {
  ...
  return <>
    <Stepper activeStep={activeStep} orientation='vertical' sx={{ width: '40%', ml: '50%',/*  */ mr: '50%' }}>
      <Step sx={{ width: '100%'}}>
        <StepLabel>
        ...
        <StepContent sx={{ width: '100%' }}>
          <Box display={'flex'} width={'100%'} justifyContent={'space-between'}>
            <NumericFormat
              ...
              InputProps={{
                endAdornment: <img width='20px' height='20px' src='./arweave-logo-for-light.png' />,
              }}
              helperText={
                <Box display={'flex'} gap={'8px'}>
                  <Typography display={'flex'} gap={'8px'} alignItems={'center'} fontWeight={'500'} color={'primary'}>
                    <InfoOutlined />
                    Looking for a way to buy AR?
                  </Typography>
                  <Typography alignItems={'center'} color='primary'>
                    <a href='https://www.coinbase.com/how-to-buy/arweave' target='_blank' rel="noreferrer"><u>Find out How.</u></a>
                  </Typography>
                </Box>
              }
            />
            {currentBalance > 0 ? <CheckCircleOutlineIcon color='success' fontSize='large' sx={{ mt: '8px'}}/> : <InfoOutlinedIcon color='warning' fontSize='large' sx={{ mt: '8px'}}/>}
          </Box>
          {lastTx && <Box mt={'16px'}>
            <Divider textAlign='left'><Typography>Last Pending Transaction</Typography></Divider>
            <Box display={'flex'} justifyContent={'space-between'} mt={'16px'}>
              <Box sx={{ display: 'flex', fontWeight: 500, gap: '8px' }}><Typography>ID:</Typography><Typography><u>{displayShortTxOrAddr(lastTx.node.id)}</u></Typography></Box>
              <Box sx={{ display: 'flex', fontWeight: 500, gap: '8px' }}><Typography>Status:</Typography><Typography color={lastTx.status === 'confirmed' ? theme.palette.success.main : theme.palette.warning.main}>{lastTx.status}</Typography></Box>
            </Box>
          </Box>}
        ...
        <StepContent sx={{ width: '100%' }}>
          <Box sx={{ width: '100%', display: 'flex', gap: '32px', flexDirection: 'column' }}>
            <Box sx={{ display: 'flex', gap: '16px' }}>
              ...
              />
              {currentUBalance > 0 ? <CheckCircleOutlineIcon color='success' fontSize='large' sx={{ mt: '8px'}}/> : <InfoOutlinedIcon color='warning' fontSize='large' sx={{ mt: '8px'}}/>}
            </Box>
            <Box>
            ...
            {lastMintTx && <Box mt={'16px'}>
              <Divider textAlign='left'><Typography>Last Pending Transaction</Typography></Divider>
              <Box display={'flex'} justifyContent={'space-between'} mt={'16px'}>
              <Box sx={{ display: 'flex', fontWeight: 500, gap: '8px' }}><Typography>ID:</Typography><Typography><u>{displayShortTxOrAddr(lastMintTx.node.id)}</u></Typography></Box>
              <Box sx={{ display: 'flex', fontWeight: 500, gap: '8px' }}><Typography>Status:</Typography><Typography color={lastMintTx.status === 'confirmed' ? theme.palette.success.main : theme.palette.warning.main}>{lastMintTx.status}</Typography></Box>
              </Box>
            </Box>}
            <Box display={'flex'} justifyContent={'space-between'} width={'100%'} gap={'16px'}>
              ...
              <Box>
              <LoadingButton variant='contained' loading={loading || isPending} loadingPosition="end" onClick={handleSwap} endIcon={<SwapHorizOutlinedIcon />}>
      ...
      {activeStep === 2 && <Box display={'flex'} flexDirection={'column'} gap={'8px'} justifyContent={'center'} p={2}>
        <Typography fontWeight={'500'}>All Done!</Typography>
        <Typography>Your Transaction is being processed by the network. This proccess can take up to 15 min and your funds will not be available until then.</Typography>
        <Typography>You can track the process <a href={`https://viewblock.io/arweave/tx/${lastMintTx?.node.id}`} target='_blank' rel="noreferrer"><u>here</u></a></Typography>
        <Box display={'flex'} justifyContent={'flex-end'} mt={'16px'}>
